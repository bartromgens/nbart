...
void Drawable::loadSurface(std::string imageloc) {
  drawablesurf = gf::load_image( imageloc , true );
}

void Drawable::setTopLeftPos() {
  pos.x = center.x - pos.w*0.5;
  pos.y = center.y - pos.h*0.5;
}

void Drawable::setCenterPos(int x, int y) {
  center.x = x;
  center.y = y;
}

void Drawable::moveCenterPos(int dx, int dy) {
  setCenterPos(center.x + dx, center.y + dy);
}

void Drawable::setSize(int w) {
